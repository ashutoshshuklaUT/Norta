...
MC_SAMPLES = 2E6  # Number of samples to compute the integral
BIN_SEARCH_LAMBDA = 0.5  # Binary search parameter (~0 computes closer to u, and ~1 computes closer to l)
ZERO_TOL = 5e-5  # Tolerance for the binary search

def sdp_optimizer(target):
    n = target.shape[0]
    X = cp.Variable((n,n), symmetric=True)
    # semi-definite constraint
    # The former constraint is to enforce positive definiteness instead of positive semi-definiteness
    constraints = [X >> 0.0001*np.eye(n,n)]
    #constraints = [X >> 0]
    for i in range(n):
...
#     # semi-definite constraint
#     constraints = [X >> 0.001 * np.eye(n, n)]
    
#     # Enforcing diagonal elements to be 1
#     for i in range(n):
#         constraints.append(X[i, i] == 1)
    
#     # Calculate percentage change
#     percentage_change = ((X - target) / target) * 100
    
#     # Compute the sum of squares of percentage change
#     cost = cp.sum_squares(percentage_change)
#     prob = cp.Problem(cp.Minimize(cost), constraints)
#     prob.solve()
#     return X.value
...
def fit_NORTA(data, n, d, CovX, F_invs=None, lambda_param=0.005, mc_samples=1E6, seed=0, output_flag=0, n_proc=8):
    """
    ...
    while not procedure_done:
        ...
        # SDP-Optimizer in place with L2 norm
        D = sdp_optimizer(D)
        try:
            C = cholesky(D, lower=True)
            procedure_done = True
        except np.linalg.LinAlgError:
            # This step is used to sequentially make the matrix positive-definite.
